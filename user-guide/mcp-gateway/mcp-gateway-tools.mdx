...
List saved generative UI templates for an agent.

**Available in:** Both MCP and Agent modes, when generative UI is enabled.

**Parameters:**
- `agent_name` (string, optional): Agent that owns the templates; omit to use the agent from the MCP connection URL
- `detailed` (boolean, optional): If `true`, include full `generative_spec` per template; if `false` (default), return summary fields only (`template_id`, description, audit fields)
...
- `agent_name` (string, optional): Owning agent; omit to use the connection URL agent
...
- `agent_name` (string, optional): Owning agent
