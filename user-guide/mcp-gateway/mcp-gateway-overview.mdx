...
1. **Get Your Endpoint**
   ...
   > **Note**: `{genai_app}` is the name of the agent or MCP that is configured in Apigene admin. The live URL uses the published agent. Use the design URL only when you want to author against the unpublished draft (skills, workflows, templates). [Which tools are design-only vs live](/user-guide/mcp-gateway/mcp-gateway-modes#design-mode-vs-live-mode-tools).
