...
- Add a custom **model ID** when your provider offers a model that is not in the default list
...
Use these when validating upgrades or diagnosing environment issues. They complement [Dashboard](/user-guide/dashboard) and [Insights](/user-guide/insights) rather than replacing them.
...
3. Enter IAM **Access Key ID**, **Secret Access Key**, and **Region**
4. Save and test — available Bedrock models appear in the Copilot model selector
...
4. Toggle models on or off for the org model selector
5. Optionally type a **model ID** and add it as a custom model
6. Save organization settings

Disabled models stay configured on the provider but are hidden from users. Custom model IDs are useful when a provider releases a new model before Apigene updates the default catalog.
