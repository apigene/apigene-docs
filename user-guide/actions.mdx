...
<Frame>
  <img src="/images/product/actions.png" alt="Actions page listing recent API interactions" style={{ borderRadius: '0.5rem' }} />
</Frame>
...
How much of the request/response body is stored depends on your organization's **Interaction Recording** policy (full bodies, headers only, or disabled). See [Settings](/user-guide/settings#interaction-recording-organization). Failed calls still keep the outbound request and a structured error response when full recording is on.

## Accessing Actions

In the **Design** workspace, open **Monitor → Actions**.
...
Click **Last 7 days** (or **7d** on mobile) to show an **activity timeline** above the actions table. The timeline summarizes API activity over the past week so you can spot spikes, errors, or unusual patterns before drilling into individual rows.

The timeline refreshes when you reload actions or change filters.

## Compare Actions

Use **Compare** to diff two action records side by side:

1. Click **Compare** in the toolbar to enter compare mode
2. Select **exactly two rows** in the table (checkboxes appear in compare mode)
3. Click **View diff** to open a fingerprint comparison (earlier vs later by timestamp)

Compare mode is useful when debugging regressions — for example, the same API call that succeeded yesterday but fails today. Exit compare mode with **Cancel**.

<Note>
You can compare at most two actions at a time. Uncheck one row before selecting a third.
</Note>
...
When the response includes a host/terminal screen (`screenText` — typical of 3270-style or MCP host sessions), a **Terminal** tab appears next to the raw and processed views. It renders the screen text, input fields, and cursor so you can inspect the host UI without reading the JSON payload. The same tab is available on shared action links.

<Note>
If your organization uses **Headers only** interaction recording, request and response **bodies are redacted**. You still see headers and payload size metadata. If recording is **Disabled**, new actions are not stored. Admins configure this under [Settings → Organization → Interaction Recording](/user-guide/settings#interaction-recording-organization).
</Note>
...
Actions feed into [Insights](/user-guide/insights):
...
- [View analytics](/user-guide/insights) to understand usage patterns
