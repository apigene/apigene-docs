...
### Version Control
Edit in **draft**, then **publish** when chat and MCP should pick up the change:
- **Save** writes the draft only — live stays on the last published version
...
1. Switch the sidebar to **Design** and open **Build → Agents**
...
**Agent Name**
- Use lowercase kebab-case: letters, numbers, and hyphens only (for example `github-bot`, `support-agent`)
- Spaces and underscores are converted to hyphens as you type
...
- MCP URLs and `@` mentions still use the agent **name**
...
3. Mention the agent using `@support-agent` (kebab-case name)
...
| Tab | Purpose |
|-----|---------|
| **Instructions** | Name, optional display name, description, icon, and system prompt |
...
| **Workflows** | List, create, visually edit, run, and simulate agent workflows |
| **Versions** | Draft vs live, publish, compare, and version history (available after first save) |
| **Setup** | Live and design MCP URLs, per-tool toggles, managed setup export |
...
| URL | What it uses |
|-----|----------------|
| `/agent/{name}/mcp` | Published **live** version (chat, Cursor, Claude, and other runtime clients) |
| `/agent/{name}/design/mcp` | Unpublished **draft** — create or delete skills, workflows, and templates until you publish |

Example design URL:

```
https://app.apigene.ai/agent/{name}/design/mcp
```

Viewers of a shared agent can connect to the live URL. Design and mutation tools are hidden unless you own the agent.

## Draft, live, and version history

After the first save, the editor and agent cards show:

- A **vN** chip for the current live version
- A **Draft** chip when unpublished changes exist

**Save** updates draft only. Chat, Tasks, and `/agent/{name}/mcp` keep using live until you publish.

### Publish

1. Open the agent → **Versions**
2. Review the draft vs live summary (instructions, apps, MCP servers, skills, context, workflows, UI templates, tools)
3. Click **Publish**, optionally add a note, and confirm

Publish creates the next numbered version and points live at it. The public MCP URL does not change.

### History

The Versions table lists each published snapshot. Open a row for details, or use the row menu to:

- **Compare with live** — side-by-side diff of instructions and attachments
...
New agents start version history on the first save (`v1`). Existing agents keep working as live; further saves stay in draft until you publish.
...
Pickers and agent cards show the optional **display name**. `@` mentions still use the kebab-case name.
...
The Agents page lists every agent you can see (no 50-item cap). Cards show the **display name** when set, live version, and a **Draft** chip when unpublished changes exist.
...
When an admin enables [Agent Marketplace](/user-guide/settings#agent-marketplace), the Agents page shows a catalog from a private GitHub repo (Claude plugin layout). Browse by industry or line of business, preview a listing, and install it as an agent in your organization.

**Try Now** installs (or reuses) the plugin agent, can prepare recommended demo applications so the agent has APIs to call, connects the agent to Copilot, and opens a **Live** chat with the agent mentioned. The first run for a line of business may take a minute and shows a progress sheet; later runs reuse what is already in place. Click the spinning **Try Now** button to reopen progress. **How it works** previews the plugin without installing. **Add to my agents** installs without opening chat.

If demo applications are not available in your deployment, **Try Now** still installs the agent as before.
...
5. Open **Versions** and **Publish** when new chats and MCP should use the change

Until you publish, existing conversations and the runtime MCP URL keep the last live version.
