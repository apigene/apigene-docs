...
2. Choose **Chat** or **Agent Builder** with the mode toggle on the home screen, or open **Agent Builder** from the sidebar / user menu
...
Type `@` in the chat input to pick a resource:

- **@agents**: Invoke a specific agent to handle the request
- **@applications / MCP**: Attach an application or native MCP server; the picker shows connection status (connected, idle, or disconnected) above the tools list
...
Copilot detects Hebrew and Arabic in messages, suggestions, and the composer and applies RTL alignment automatically. Code blocks stay LTR for readability.
...
**Use Mentions**
- Type `@` to pick an agent, application, MCP server, or workflow
- The picker shows MCP connection status (connected, idle, or disconnected) above the tools list
