...
Open the list from the **Design** workspace: **Build → Applications**.
...
- **OpenAPI based** — import from URL, file, **Postman collection**, or an **OpenLegacy Hub** project
...
<Note>
Conversion depends on collection structure. Nested folders become tags where possible. Review generated operations after install and reindex if tool metadata looks incomplete. If conversion fails, the error names the problem (invalid JSON, not a Postman collection, unresolved variables, or comments / stray quotes in the export).
</Note>

### Method 4: From OpenLegacy Hub

Install a Hub project's OpenAPI as an application. Hub must be connected under **Settings → Organization → Open Legacy** (including **Use in Agent Builder**). See [OpenLegacy Hub](/user-guide/openlegacy).

1. Navigate to **Applications** → **+ Install Application**
2. Choose **OpenAPI based**, then **OpenLegacy Hub**
3. Search and select a Hub project (method count, last updated, and an **Installed** badge appear when relevant)
4. Click **Install** (or **Open** if that project is already installed)

The Hub card is shown only when the organization can reach Hub. Installation uses the same index and metadata flow as a file or URL import.

### Method 5: Native MCP Server
...
**Search and Filter**
- Search by operation name or description
- Filter by HTTP method (GET, POST, etc.)
- Filter by **path prefix** (e.g. `/api/v1/users`)
- Filter by one or more **tags**
- Clear filters to reset the list; pagination respects the active filter set
...
### Agentic metadata list

Access via **Applications** > [Application Name] > **Metadata** tab

Browse and manage generated agentic metadata for each operation:

**Search and Filter**
- Search by operation ID or description
- Filter by metadata **type**
- Filter by **priority rating** (1 = highest priority, 10 = lowest)
- Results sort by rating so high-value operations surface first

Use these filters on large specs to find operations that need description or rating edits before reindexing.
